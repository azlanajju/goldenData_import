...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["excelFile"])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO Customers (CustomerUniqueID, Name, Contact, Email, PasswordHash, Address, ProfileImageURL, BankAccountName, BankAccountNumber, IFSCCode, BankName, PromoterID, ReferredBy, Status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        for ($row = 2; $row <= $highestRow; $row++) {
            ...
            try {
                $stmt->execute([$customerUniqueID, $name, $contact, $email, $passwordHash, $address, $profileImageURL, $bankAccountName, $bankAccountNumber, $ifscCode, $bankName, $promoterID, $referredBy, 'Active']);
                $successCount++;
            } catch (PDOException $e) {
                $errorCount++;
                $errorMessage = "Error in row $row: " . $e->getMessage() . "\n";
                $errorMessage .= "Data: CustomerUniqueID: $customerUniqueID, Name: $name, Contact: $contact, Email: $email\n";
                $errorMessage .= "----------------------------------------\n";
